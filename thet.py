import base64           #编码解码
import urllib.parse     #url解析和引用
import hmac             #HMAC是密钥相关的哈希运算消息认证码（Hash-based Message Authentication Code）的缩写
from hashlib import sha1        #摘要算法
import requests         #处理URL资源
import uuid             # 通用唯一标识符
import time
import hmac, ssl        #hex-based message authentication code 哈希消息认证码
import re
...
data = re.search('<Content>(.*)</Content>', r.text,re.S)
print(data.group(1))